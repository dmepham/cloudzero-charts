...
{{- $requiredTargetMetricLabels := tuple "__address__" "__scheme__" "__metrics_path__" "__param.*" "__meta.*" "_.*" }}
{{- $requiredCZMetricLabels := tuple "board_asset_tag" "container" "created_by_kind" "created_by_name" "image" "instance" "namespace" "node" "node_kubernetes_io_instance_type" "pod" "product_name" "provider_id" "resource" "unit" "uid" -}}
{{- $total := concat .Values.additionalMetricLabels $requiredCZMetricLabels $requiredTargetMetricLabels -}}
{{- $result := join "|" $total -}}
{{- $result -}}
{{- end -}}
